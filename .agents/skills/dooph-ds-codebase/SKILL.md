...
src/
  ...
  components/
    ...
    SegmentedTabSelect/
    ShapeButton/
    Shapes/                     ← 12 shape primitives; 5 of them back ShapeButton
    Sheet/
...
| `ShapeButton`        | `ShapeButton/ShapeButton.tsx`     | `Slot` | `ShapeButtons` (shape) × `ShapeButtonVariant` | ✅      |
...
All icons live in `src/components/Icons/`. They extend `BaseIcon` with an `IconSize` enum (`sm` 12 / `rg` 14 / `md` 16 / `lg` 18, backed by `--ui-icon-sm`/`-rg`/`-md`/`-lg`; stroke from `--ui-icon-stroke-width`, 1.5). Renamed in 5.4 from `tiny`/`standard`/`medium` — the old names and the 20px `--ui-icon-large` are gone. The `Icons/index.ts` is auto-generated by `scripts/generate-icon-exports.mjs`.
